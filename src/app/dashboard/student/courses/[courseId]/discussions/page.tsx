...
import { createReplyAction, createThreadAction, deleteReplyAction, deleteThreadAction } from "@/app/dashboard/discussions/actions";
...
export default async function StudentCourseDiscussionsPage({
  ...
  const redirectTo = `/dashboard/student/courses/${courseId}/discussions`;
  ...
  return (
    <div className="grid gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-text-secondary">Ask questions and discuss course material with classmates and your instructor.</p>
      </div>
      ...
      <div className="grid gap-4">
        {((threads ?? []) as ThreadRow[]).length === 0 ? (
          <p className="text-sm text-text-secondary">No discussions yet — be the first to post.</p>
        ) : (
      ...
      <Link className="w-fit text-sm font-semibold text-primary-hover" href={`/dashboard/student/courses/${course.id}`}>
        Back to course
      </Link>
    </div>
  );
